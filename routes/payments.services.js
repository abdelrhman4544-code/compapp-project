...
con.query("INSERT INTO payments (`order_id`, `total_price`,`payment_method`,`payment_status`) VALUES (?,?,?,?)",
[req.body.order_id,req.body.total_price,req.body.payment_method,req.body.payment_status], function (err, result,fields) {
     if (err) throw err;
...
router.put('/payments',(req,res)=>{
    ...
    var payment_id= req.query.payment_id;
    con.query("UPDATE payments SET  `order_id` = ? , `total_price` = ? ,`payment_method` = ?,`payment_status` = ? WHERE payment_id = " + payment_id ,
    [req.body.order_id,req.body.total_price,req.body.payment_method,req.body.payment_status], function (err, result, fields) {
       if (err) throw err;
